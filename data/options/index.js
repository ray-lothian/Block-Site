...
const modalContentEl = modalEl.getElementsByClassName("modal-content")[0];
const modalCloseEl = modalEl.getElementsByClassName("modal-close")[0];
...
[...document.querySelectorAll("[data-i18n]")].forEach(e => {
  if (e.nodeName !== "SUP") {
    e[e.dataset.i18nValue || "textContent"] = chrome.i18n.getMessage(
      e.dataset.i18n
    );
  } else {
    e.addEventListener("click", e => {
      modalContentEl[e.target.dataset.i18nValue || "textContent"] =
        localizationBase[e.target.dataset.i18n] &&
        localizationBase[e.target.dataset.i18n].message;
...
function add(hostname) {
  ...
  tr.dataset.pattern = node.querySelector(
    "td:nth-child(1)"
  ).textContent = wildcard(hostname);
  ...
  node.querySelector('[data-cmd="remove"]').value = chrome.i18n.getMessage(
    "options_remove"
  );
...
const init = (table = true) =>
  chrome.storage.local.get(prefs, ps => {
    ...
    document.querySelector("#schedule [name=start]").value =
      prefs.schedule.time.start;
    document.querySelector("#schedule [name=end]").value =
      prefs.schedule.time.end;
    document.querySelector(
      "#schedule [name=days]"
    ).value = prefs.schedule.days.join(", ");
    document.querySelector("#schedule [name=hostname]").value = "";
    document.querySelector("[data-cmd=unlock]").disabled =
      prefs.password === "";
    document.querySelector('[data-cmd="save"]').disabled =
      prefs.password !== "";
    document.querySelector('[data-cmd="export"]').disabled =
      prefs.password !== "";
    document.querySelector('[data-cmd="import-json"]').disabled =
      prefs.password !== "";
...
document
  .querySelector('#schedule [name="hostname"]')
...
document.addEventListener("click", e => {
  ...
  if (cmd === "remove") {
    ...
    let schedule = {
      ...
      days: document
        .querySelector("#schedule [name=days]")
        .value.split(/\s*,\s*/)
        .map(s => {
          return days
            .filter(d =>
    ...
    chrome.storage.local.get(null, prefs => {
      const blob = new Blob([JSON.stringify(prefs, null, "\t")], {
